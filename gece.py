  ...
  if event.is_private:
    async for usr in client.iter_participants(event.chat_id):
     ...
     await event.reply(f"{ad} {startmesaj}", buttons=(
                     ...
                     [Button.url('📣 Söhbət Qrupu', f'https://t.me/{WerabliAnlar}')],
                      [Button.url('📣 Kanal', f'https://t.me/{ASOresmi}')],
                       [Button.url('👨🏻‍💻 Sahib', f'https://t.me/{ismiyev95}')]
                    ),
                    link_preview=False)
...
async def handler(event):
    async for usr in client.iter_participants(event.chat_id):
     ...
     await event.edit(f"{ad} {startmesaj}", buttons=(
                      [
                       Button.inline("✍ Əmrlər",Button.komek")
                      ],
                      [Button.url('🌱 Məni Qrupa Əlavə Et', f'https://t.me/{ASOTagger_Bot}?startgroup=a')],
                     [Button.url('📣 Söhbət Qrupu', f'https://t.me/{WerabliAnlar}')],
                      [Button.url('📣 Kanal', f'https://t.me/{ASOresmi}')],
...
async def mentionall(event):
  ...
  if mode == "text_on_cmd":
    await client.send_message(event.chat_id, "❄️ Tağ Başladı\n⏱️ İnterval - 2 saniyə",
                    buttons=(
                      [
                      Button.url('📣 Sohbet Qrupu', f'https://t.me/{WerabliAnlar}')
                      ]
                    )
                  ) 
...
print(">> Bot çalışmaktadur merak etme 🚀 @ bilgi alabilirsin <<")
